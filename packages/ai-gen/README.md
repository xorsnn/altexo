...
(image), **Veo 3.1** (video), and **Kling 3** (video) over their native APIs and
writes deterministic outputs into `out/<project>/YYYY-MM-DD_HHMM_<slug>_<model>/`
with a `manifest.json` reproducibility receipt.
...
- No audio out of Kling — pair with a music bed in your editor.
